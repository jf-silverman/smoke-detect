...
def grouped_split(
    ...
    *,
    ...
    Sites are assigned greedily largest-first into whichever partition is furthest
    below its image-count target. Greedy beats random assignment here because site
    sizes are heavily skewed -- a few towers dominate -- and a random draw can hand
    the test set a single site or wildly miss the target fractions.
    """
    df = prepare(df)
