...
non-maximum suppression (NMS). Second, most of an image is background, so detectors fight a
severe **foreground/background imbalance** (addressed by focal loss and hard-negative mining;
Lin et al., 2017b).

### 1.2 The architectural families

**Two-stage detectors (region-based).** First propose candidate regions, then classify and refine
each. The R-CNN lineage — R-CNN (Girshick et al., 2014) → Fast R-CNN (Girshick, 2015) → Faster
...
**Transformer / set-prediction detectors.** DETR (Carion et al., 2020) reframed detection as
direct **set prediction**: a transformer emits a fixed set of predictions matched to ground truth
by bipartite (Hungarian) matching — **no anchors and no NMS**. It was elegant but slow to train;
...
al., 2016), EfficientNet (Tan & Le, 2019), and the transformer backbones ViT (Dosovitskiy et al.,
2021) and Swin (Liu et al., 2021). Feature Pyramid Networks (Lin et al., 2017a) let a detector
use multiple scales at once — important for small objects like distant smoke.

### 1.3 How performance is measured (and a caveat)

The standard metric is **mean Average Precision (mAP)**: average precision (area under the
precision-recall curve) per class, meaned over classes and over IoU thresholds (COCO averages IoU
0.50–0.95; Lin et al., 2014). **Intersection-over-Union (IoU)** measures box overlap.
...
4. **Native video models.** 3D CNNs, ConvLSTMs, and video transformers — TimeSformer (Bertasius
...
The project's own temporal experiments are a lightweight instance of category 3/4 (a GRU over a
