...
- **No boundary.** Smoke is semi-transparent and diffuse; a pixel is *partly* smoke. This is
  ...
  where a plume ends, which puts an **annotation-noise ceiling** on IoU — a perfect model
  cannot beat inter-annotator agreement. This is why the serious papers abandon standard
  metrics: SKLFS evaluates at **AP@0.1**, and PyroNear drops mAP entirely in favor of
...
**Report instead:**
1. **Precision, recall, F1, AUC-PR** — with the full PR curve, and an explicitly justified
   operating point.
